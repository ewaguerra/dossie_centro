...
function makeMockPanel(checkboxes) {
  ...
  checkboxes.forEach(function (cb) {
    cb.matches = function (sel) {
      return (
        typeof sel === 'string' &&
        sel.indexOf('checkbox') !== -1 &&
        sel.indexOf('data-layer-id') !== -1
      );
    };
    cb.dispatchEvent = function () {
      var handlers = panel._listeners.change || [];
...
describe('projeto_centro — sanity checks', () => {
  ...
  it('protocolo-phase: fail-closed quando gates ainda nao carregaram', () => {
    const phase = read('centro/features/protocolo-phase.js');
    assert.ok(phase.includes('arePhaseGatesReady'), 'export arePhaseGatesReady');
    assert.ok(phase.includes('if (!gatesData) return MAX_PHASE'), 'getMinPhaseForLayer fail-closed');
    assert.ok(phase.includes('if (!gatesData) return false'), 'isLayerPhaseUnlocked fail-closed');
    assert.ok(phase.includes('centro:arg-state-changed'), 'setPhase dispara arg-state-changed');
  });
  ...
  it('sidebar-events usa delegacao para re-render idempotente', () => {
    const events = read('centro/ui/sidebar-events.js');
    assert.ok(events.includes('data-centro-layer-wire'), 'flag de wire unico no painel');
    assert.ok(events.includes('panel.addEventListener("change"'), 'delegacao change no painel');
  });
  ...
  it('scripts/lib/python-cmd.mjs resolve python cross-platform', () => {
    assert.ok(exists('scripts/lib/python-cmd.mjs'), 'helper python ausente');
    const helper = read('scripts/lib/python-cmd.mjs');
    assert.ok(helper.includes('resolvePythonCommand'), 'resolvePythonCommand exportado');
    assert.ok(helper.includes('PYTHONIOENCODING'), 'encoding utf-8 no spawn');
  });
